...
class MQClient {
   ...
   * ```json
   */
  async request (method, resource, type, requestBody, opts = {}, retryCount = 2) {
    ...
    const response = await httpx.request(url, requestOpts).catch(err => {
      if (this.options.retryOnTimeout && err.message.startsWith('ReadTimeout')) {
    ...
    if (responseBody && (contentType.startsWith('text/xml') || contentType.startsWith('application/xml'))) {
      ...
      if (responseData.Error) {
        ...
        if (errorcode === 'InternalError' && retryCount > 0) {
