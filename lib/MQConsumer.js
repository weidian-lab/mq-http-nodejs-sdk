...
class MQConsumer extends EventEmitter {
  ...
  async subscribe (onMsg, options = {}) {
    ...
    while (this.startedAt) {
      ...
      if (clusterPendingLimit) {
        try {
          const clusterPendingCount = await incrClusterPendingCount(this.consumer)
          if (clusterPendingCount > clusterPendingLimit) {
            await decrClusterPendingCount(this.consumer)
      ...
      try {
        ...
        if (msgsRet.code === 200) {
          ...
          msgsRet.body.map(async message => {
            ...
            try {
              ...
              try {
                if (clusterPendingLimit) {
                  await decrClusterPendingCount(this.consumer).catch(err => {
        ...
        if (clusterPendingLimit) {
          await decrClusterPendingCount(this.consumer).catch(e => {
