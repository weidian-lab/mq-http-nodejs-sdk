...
class MQConsumer extends EventEmitter {
  ...
  async subscribe (onMsg, options = {}) {
    ...
    const {
      ...
      clusterPendingLimit, incrementPendingCount, checkDuplicatedMsg
    } = { ...this.client.options, ...options }
    if (clusterPendingLimit) {
      assert(incrementPendingCount, 'required incrementPendingCount')
    ...
    while (this.startedAt) {
      ...
      if (clusterPendingLimit) {
        try {
          const clusterPendingCount = await incrementPendingCount(this.consumer, 1)
          if (clusterPendingCount > clusterPendingLimit) {
            await incrementPendingCount(this.consumer, -1)
      ...
      try {
          ...
          msgsRet.body.map(async message => {
            ...
            try {
              ...
              if (checkDuplicatedMsg) {
                isDuplicatedMsg = await checkDuplicatedMsg(`${this.consumer}_${MessageId}`).catch(err => {
              ...
              try {
                if (clusterPendingLimit) {
                  await incrementPendingCount(this.consumer, -1).catch(err => {
                    this.logger.error('[mqtt] incrementPendingCount error', err)
                  })
                }
        ...
        if (clusterPendingLimit) {
          await incrementPendingCount(this.consumer, -1).catch(e => {
            this.logger.error('[mqtt] incrementPendingCount error', e)
          })
        }
      }
    }
